...
### ปุ่มทางเข้าเว็บ (ระดับแบรนด์)
ใส่ที่ระดับบนสุดของ `brand.json` → ปุ่มโทนเข้มขอบทองใต้โลโก้ + โผล่คู่ปุ่มโหลดในแถบลอยด้านล่าง:
```json
"siteUrl": "https://.../m", "siteLabel": "ເຂົ້າສູ່ເວັບໄຊ້ Brand", "siteShort": "ເຂົ້າເວັບ"
```
`siteShort` = ข้อความสั้นที่ใช้ในแถบลอย (พื้นที่จำกัด)

**แถบปุ่มลอยด้านล่าง** ทำงานอัตโนมัติ: ซ่อนตอนเห็นปุ่มหลักอยู่แล้ว → โผล่เมื่อปุ่มเลื่อนพ้นจอ → เว้นที่ท้ายหน้าไม่ให้ทับ footer
